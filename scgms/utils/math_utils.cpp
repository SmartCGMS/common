...
bool Calculate_Signal_Stats(std::vector<double>& series, scgms::TSignal_Stats& signal_error) {
	...
	// 4. calculate stddev
	{
		...
		signal_error.exc_kurtosis = central_moment_4 * corrected_count / (square_error * square_error) - 3.0;
		signal_error.skewness = central_moment_3 / (square_error * signal_error.stddev);
	}
