...
bool Has_Hexa_Prefix(const char* str) {
	return Is_Hexa_Double(str);
}

bool Has_Hexa_Prefix(const std::string& str) {
	return Is_Hexa_Double(str.c_str());
}

bool Has_Hexa_Prefix(const wchar_t* wstr) {
	return Is_Hexa_Double(wstr);
}

bool Has_Hexa_Prefix(const std::wstring& wstr) {
	return Is_Hexa_Double(wstr.c_str());
}

bool rtl0_isspace(const char str) {
	return std::isspace(static_cast<unsigned char>(str)) != 0;
}

bool rtl0_isspace(const wchar_t str) {
	return std::iswspace(static_cast<wint_t>(str)) != 0;
}

double rtl0_str_dbl(const char* str, char** end_ptr) {
	return strtod(str, end_ptr);
}

double rtl0_str_dbl(const wchar_t* str, wchar_t** end_ptr) {
	return wcstod(str, end_ptr);
}

uint64_t rtl0_str_uint64(const char* str, char** end_ptr, int base) {
	return strtoul(str, end_ptr, base);
}

uint64_t rtl0_str_uint64(const wchar_t* str, wchar_t** end_ptr, int base) {
...
namespace {
	...
	double convert_str_2_double(const C* wstr, bool& ok) {
		...
		if (end_char != nullptr) {
			if (*end_char == TNumeric_Chars<C>::dot) {
				value = try_convert(TNumeric_Chars<C>::dot, TNumeric_Chars<C>::coma);
			}
			else if (*end_char == TNumeric_Chars<C>::coma) {
				value = try_convert(TNumeric_Chars<C>::coma, TNumeric_Chars<C>::dot);
			}
		}
...
double str_2_dbl(const std::wstring& str) {
	return str_2_dbl(str.c_str());
}
...
namespace {
	...
	std::basic_string<C> rtl_dbl_2_str(const double val, const bool save_hexa) {
		...
		if (!save_hexa) {
			...
			const auto iter = symbols.find(cls);
...
uint64_t str_2_uint(const std::string& str, bool& ok) {
	return str_2_uint(str.c_str(), ok);
}
...
std::wstring GUID_To_WString(const GUID& guid) {
	...
	const TGUID_Overlay& guid_overlay = *reinterpret_cast<TGUID_Overlay*>(const_cast<GUID*>(&guid));
