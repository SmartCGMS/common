...
public:
	...
	TSolution<n>(const Eigen::ArrayBase<OtherDerived>& other) : Eigen::Array<double, 1, n, Eigen::RowMajor>(other) { }
	TSolution<n> & operator= (const TSolution<n>& other) {
		this->Base::operator=(other);
		return *this;
	}

	template<typename OtherDerived>
	TSolution<n> & operator= (const Eigen::ArrayBase <OtherDerived>& other) {
		this->Base::operator=(other);
		return *this;
	}

	TSolution<n> Compose(const TSolution<n> &) const {
		return TSolution<n>(*this);
	}

	TSolution<n> Decompose(const TSolution<n>& src) {
		*this = src;
		return TSolution<n>(src);
	}

	virtual HRESULT set(double *begin, double *end) final {
		Eigen::Array<double, 1, n, Eigen::RowMajor> &raw = *this;
		size_t i = 0;
...
using CSteil_Rebrin_Diffusion_Prediction_Solution = TSolution< steil_rebrin_diffusion_prediction::param_count>;
using CDiffusion_Prediction_Solution = TSolution< diffusion_prediction::param_count>;
using CGeneric_Solution = TSolution<Eigen::Dynamic>;
