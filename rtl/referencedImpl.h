...
/*
	...
	template<typename T>
	class IVector_Container_Iterator {
	private:
		T *mBegin = nullptr, *mEnd = nullptr;

	public:
		IVector_Container_Iterator(refcnt::IVector_Container<T>* container) {
			container->get(&mBegin, &mEnd);
		}
	...
	// wrapper element to allow conversion during iteration
	template<typename T, typename C>
	class IVector_Container_Iterator_Element final {
	private:
		T* mElement;

	public:
		IVector_Container_Iterator_Element(T* event) : mElement(event) {
			//
		}

		IVector_Container_Iterator_Element& operator++() {
			mElement++;
			return *this;
		}
		...
		IVector_Container_Iterator_Element& operator=(T* evt) {
			mElement = evt;
		}

		bool operator==(IVector_Container_Iterator_Element& evt) {
			return (mElement == evt.mElement);
		}
	...
	// convertor iterator class to make e.g. range based for loops work, even with value-constructible wrappers like shared pointers
	// example: for (auto& element : IVector_Container_Iterator<Element*, SElement>(container))
	// NOTE: it is crucial not to modify the underlying container during iteration, as it may reallocate memory and invalidate begin and end pointers
	template<typename T, typename C>
	class IVector_Container_Casting_Iterator {
	private:
		T *mBegin, *mEnd;

	public:
		IVector_Container_Casting_Iterator(refcnt::IVector_Container<T>* vec) {
			vec->get(&mBegin, &mEnd);
		}

		IVector_Container_Iterator_Element<T, C> begin() {
			return IVector_Container_Iterator_Element<T, C>(mBegin);
		}

		IVector_Container_Iterator_Element<T, C> end() {
